...
func (c *Client) AgreementQuery(param AgreementQuery) (result *AgreementQueryRsp, err error) {
...
func (c *Client) AgreementUnsign(param AgreementUnsign) (result *AgreementUnsignRsp, err error) {
	err = c.doRequest("POST", param, &result)
	return result, err
}

// AgreementExecutionPlanModify 周期性扣款协议执行计划修改接口 https://docs.open.alipay.com/api_2/alipay.user.agreement.executionplan.modify
func (c *Client) AgreementExecutionPlanModify(param AgreementExecutionPlanModify) (result *AgreementExecutionPlanModifyRsp, err error) {
	err = c.doRequest("POST", param, &result)
	return result, err
}
...
func (c *Client) DecodePhoneNumber(data string) (result *MobileNumber, err error) {
	if err = c.decode([]byte(data), "response", true, &result); err != nil {
		return nil, err
	}
	return result, nil
}
